...
{esta función recibe la lista y devuelve una lista}
...
{8. Un procedimiento llamado borrarOcurrencias, el cual recibirá 4 argumentos:
...
begin
  ...
  writeln;
      ...
      if (opcion3Usuario='PRIMER') or (opcion3Usuario='primer') then begin
        apuntador:=lista;
        while apuntador<>NIL do begin
          if (apuntador^.numero=numeroUsuario) and (repetido=false) then begin
            apuntador:=apuntador^.siguiente;
            siguiente:=apuntador;
            repetido:=true;
            if repetido then begin
             borrarPrimeraOcurrencia (numeroUsuario, siguiente);
             exit;  {!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!}
            end;
          end else begin
              apuntador:=apuntador^.siguiente;
